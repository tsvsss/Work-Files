...
--              AND case_status = 'O'
              AND case_state NOT IN ('D', 'E')
              AND remove_special_char (name_screened) =
                                         remove_special_char (p_name_screened)
...
--              AND case_status = 'O'
   ...
   BEGIN
      ...
      THEN
         -- dept := 'Seafarers';     -- (FIN: ' || TO_CHAR (l_source_id) || ')';
      ...
      THEN
         dept := 'Non-RMI Request - ' || l_source_id;
      ELSE
         dept := 'Unknown';
      END IF;
