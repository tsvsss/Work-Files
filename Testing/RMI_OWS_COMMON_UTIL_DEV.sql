...
AS
   ...
   * 07-JUL-2020 IRI                 1.55               TSUAZO              07-JUL-2020 B  Fix where clause for alert mismatch              *
*******************************************************************************************************************************************************/
...
END create_ows_generic;
         ...
         SELECT   b.*, xref.start_date, xref.end_date, xref.master_id,
             ...
             FROM xwrl_party_xref xref,
                  (SELECT ID, relationship_type, entity_type, state, status,
                          ...
                          wc_screening_request_id, department,priority, document_type
         ...
         LOOP
            ...
            LOOP
               ...
               t_data (ii).wc_screening_request_id :=
                                                   rec.wc_screening_request_id;
              ----
             t_data (ii).department := rec.department;
                  t_data (ii).priority := rec.priority;
                  t_data (ii).document_type := rec.document_type;
            ----
   ...
   IS
      entity_data        rmi_ows_common_util.screening_tab;
      entity_data1       rmi_ows_common_util.screening_tab;
      l_req              rmi_ows_common_util.ows_request_rec;
      l_id               NUMBER;
      x_return_code      VARCHAR2 (100);
      x_retrun_msg       VARCHAR2 (500);
   ...
   BEGIN
      ...
      DBMS_OUTPUT.put_line ('Start');
      -- Fetch all related entities into table type variable
      rmi_ows_common_util.query_cross_reference ('CORP_MAIN',
                                                 'CORP_ID',
                                                 p_corp_id,
                                                 entity_data
                                                );
      ...
      LOOP
         ...
         IF entity_data (i).source_id = p_esr_id
         THEN
            
            l_new_batch := 'Y';
            entity_data1(1) := entity_data(i);
      ...
      --
      IF l_new_batch = 'Y' AND entity_data1.COUNT > 0
      THEN
         
         IF entity_data1 (1).batch_id IS NOT NULL THEN
         l_batch_id := entity_data1(1).batch_id;
         ELSE
         SELECT xwrl_batch_seq.NEXTVAL
           INTO l_batch_id
           FROM DUAL;
         ...
         -- Loop through each entity and create OWS vetting
         FOR i IN 1 .. entity_data1.COUNT
         LOOP
            IF NVL (entity_data1 (i).tc_excluded, 'N') != 'Y'
            THEN
               l_id := NULL;
               x_return_code := 'SUCCESS';
               x_retrun_msg := NULL;
               ...
               IF entity_data1 (i).entity_type = 'INDIVIDUAL'
               THEN
                  l_req.entity_type := 'INDIVIDUAL';
                  l_req.source_table := entity_data1 (i).source_table;
                  l_req.source_table_column :=
                                          entity_data1 (i).source_table_column;
                  l_req.source_id := entity_data1 (i).source_id;
                  l_req.city_id := entity_data1 (i).city_of_residence_id;
                  --
                  --
                  l_req.name_screened := entity_data1 (i).full_name;
                  l_req.full_name := entity_data1 (i).full_name;
                  l_req.last_name := entity_data1 (i).family_name;
                  l_req.first_name := entity_data1 (i).given_name;
                  l_req.gender := entity_data1 (i).sex;
                  l_req.date_of_birth := entity_data1 (i).date_of_birth;
                  l_req.residence_country_code :=
                     get_country_iso_code (entity_data1 (i).country_of_residence
                                          );
                  l_req.nationality :=
                     get_country_iso_code
                                      (entity_data1 (i).citizenship_country_code
                                      );
                  l_req.passport_issuing_country_code :=
                     get_country_iso_code
                                 (entity_data1 (i).passport_issuing_country_code
                                 );
                  l_req.passport_number := entity_data1 (i).passport_number;
                  l_req.master_id := entity_data1 (i).master_id;
                  l_req.alias_id := entity_data1 (i).alias_id;
                  l_req.xref_id := entity_data1 (i).xref_id;
                  l_req.wc_screening_request_id :=
                                       entity_data1 (i).wc_screening_request_id;
                  l_req.department := entity_data1 (i).department;
                  l_req.priority := entity_data1 (i).priority;
                  l_req.document_type := entity_data1 (i).document_type;
            ...
            THEN
               ...
               UPDATE xwrl_party_xref
                        SET last_update_date = SYSDATE,
                            last_updated_by = l_user_id,
                            last_update_login = l_login_id
                      WHERE ID = entity_data1 (i).xref_id
                        AND NVL (tc_excluded, 'N') != 'Y';
...
BEGIN
   ...
   LOOP
      ...
      fnd_file.put_line(fnd_file.LOG,'Master ID in esr_user_master'||l_master_id);
	  ...
	  then
		...
		l_fullname   := recesr.VOID_ESR_USR;
	       ...
	       BEGIN
            ...
            fnd_file.put_line(fnd_file.LOG,'esr cross ref Esr'||recesr.esr_id);
            BEGIN
               SELECT MAX (ID)
                 INTO lv_id
                 FROM xwrl_party_master xpm
         ...
         AND relationship_type = 'Primary'
            ...
            INSERT INTO xwrl_party_xref
                 ...
                 VALUES (p_id, lv_id,
                        ...
                        fnd_file.put_line(fnd_file.LOG,'p_id::'|| p_id|| ' lv_id::'||lv_id);
            ...
            LOOP
             ...
             --xwrl_ows_utils.changetoowsstate(p_alert_in_tbl (i).p_to_state);
                  ...
                  apps.mt_log_error (  'Update xwrl_response_ind_columns ');
                     ...
                     apps.mt_log_error (  'Update xwrl_response_ind_columns : ID '|| r_ind_cols.ID|| ' AND UID' ||l_listid );
                     ...
                     SELECT xwrl_alert_notes_seq.NEXTVAL,
                      ...
                      WHERE 1 = 1
                       ...
                       and n.line_number not in (select x.line_number from xwrl_alert_notes x where x.request_id = r_ind_cols.request_id and x.alert_id = r_ind_cols.alertid)
                        ...
                        apps.mt_log_error (  'Insert xwrl_alert_notes : request_id '|| p_request_id|| ' AND alert_id' || p_alert_in_tbl (i).p_alert_id );
             ...
             --xwrl_ows_utils.changetoowsstate(p_alert_in_tbl (i).p_to_state);
                  ...
                  apps.mt_log_error (  'ENTITY Update xwrl_response_entity_columns ');
                     ...
                     UPDATE xwrl_response_entity_columns
                      ...
                      apps.mt_log_error (  'ENTITY Update xwrl_response_entity_columns : ID '|| r_ent_cols.ID|| ' AND UID' ||l_listid );
                      ...
                      WHERE 1 = 1
                       ...
                       and n.line_number not in (select x.line_number from xwrl_alert_notes x where x.request_id = r_ent_cols.request_id and x.alert_id = r_ent_cols.alertid)
                        ;
                         ...
                         apps.mt_log_error (  'ENTITY Insert xwrl_alert_notes : request_id '|| p_request_id|| ' AND alert_id' || p_alert_in_tbl (i).p_alert_id );
