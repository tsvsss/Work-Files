...
* 02-JUN-2020 IRI 1.11 TSUAZO 02-JUN-2020 B Add missing commit statement
  ...
  * 23-JUL-2020 IRI                 1.16               TSUAZO              23-JUL-2020 B  process alert remove OWS status check       *              
    ...
    * 24-JUL-2020 IRI                 1.18               TSUAZO              24-JUL-2020 B  Resolved overloading procedure issue     *      
          * 26-JUL-2020 IRI              1.19               TSUAZO       26-JUL-2020 B  Fix alert notes                    *
                    * 27-JUL-2020 IRI              1.20               TSUAZO       27-JUL-2020 B  Add exception handler to update statement                    *
* 06-AUG-2020 IRI              1.21               TSUAZO       06-AUG-2020 B  Capture x_state in log                    *                    
********************************************************************************************************************/
      ...
      THEN
         ...
         /*
            ...
            */
            ...
            APPS.mt_log_error('XWRL_OWS_UTILS Batch ID: '||v_batch_id||' Request ID: '||p_request_id||' Alert ID: '||l_alert_in_tbl (i).alert_id||' To State: '|| l_alert_in_tbl (i).to_state);
         ...
         /*
            ...
            */
            ...
            /*
               ...
               APPS.mt_log_error('XWRL_OWS_UTILS Batch ID: '||v_batch_id||' Request ID: '||p_request_id||' Alert ID: '||l_alert_out_tbl (i).alert_id||' Key Label: '|| l_alert_out_tbl (i).key_label||' Old State: '|| l_alert_out_tbl (i).old_state||' New State: '|| l_alert_out_tbl (i).new_state||' Status: '||l_alert_out_tbl (i).status||' Error: '||l_alert_out_tbl (i).err_msg);                      
         ...
         IF     v_path = 'INDIVIDUAL'   AND l_alert_out_tbl (i).new_state IS NOT NULL THEN
            ...
            SELECT SUBSTR (v_key_label, 1, INSTR (v_key_label, ':', 1) - 1)
              ...
              FROM DUAL;
               ...
               APPS.mt_log_error('XWRL_OWS_UTILS Batch ID: '||v_batch_id||' Request ID: '||p_request_id||' Alert ID: '||l_alert_out_tbl (i).alert_id||' Case ID: '||v_case_key);

            SELECT TO_NUMBER (SUBSTR (v_key_label, INSTR (v_key_label, ':', -1) + 1,LENGTH (v_key_label)))
              INTO v_list_id
              FROM DUAL;

            APPS.mt_log_error('XWRL_OWS_UTILS Batch ID: '||v_batch_id||' Request ID: '||p_request_id||' Alert ID: '||l_alert_out_tbl (i).alert_id||' List ID: '||v_list_id);

                  UPDATE xwrl_response_ind_columns
                     --SET x_state = changeowsstate (l_alert_out_tbl (i).new_state),
                  SET x_state = listrecordtype|| SUBSTR(changeowsstate (l_alert_out_tbl (i).new_state),4),
                  ...
                  -- tsuazo 20200717 THIS NEED TO BE SYNCHRONIZED WITH RMI_OWS_COMMON_UTIL.SYNC_MATCHES
                     ...
                     APPS.mt_log_error('XWRL_OWS_UTILS Batch ID: '||v_batch_id||' Request ID: '||p_request_id||' Alert ID: '||l_alert_out_tbl (i).alert_id||' State: '||SUBSTR(changeowsstate (l_alert_out_tbl (i).new_state),4));

                  COMMIT;

                  EXCEPTION  -- TSUAZO 07/31/2020 ADD EXCEPTION HANDLER
                  WHEN OTHERS THEN
                  v_error_msg := SQLERRM;
                  APPS.mt_log_error('XWRL_OWS_UTILS Batch ID: '||v_batch_id||' Request ID: '||' Alert ID: '||l_alert_out_tbl (i).alert_id||p_request_id||' Error: '||v_error_msg);
...
ELSIF     v_path = 'ENTITY'    AND l_alert_out_tbl (i).new_state IS NOT NULL THEN
            ...
            APPS.mt_log_error('XWRL_OWS_UTILS Batch ID: '||v_batch_id||' Request ID: '||p_request_id||' Alert ID: '||l_alert_out_tbl (i).alert_id||' List ID: '||v_list_id);
                  ...
                  WHEN OTHERS THEN
                  v_error_msg := SQLERRM;
                  APPS.mt_log_error('XWRL_OWS_UTILS Batch ID: '||v_batch_id||' Request ID: '||p_request_id||' Alert ID: '||l_alert_out_tbl (i).alert_id||' Error: '||v_error_msg);
            ...
            --- PROCESS NOTES XREF
               ...
               -- PROCESS NOTES
for c1rec in c2(r_cols.master_id, r_cols.alias_id, r_cols.xref_id, r_cols.listid) loop
