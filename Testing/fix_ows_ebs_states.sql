...
select col.id,col. request_id, req.batch_id, col.rec, req.case_state, req.case_status, req.case_workflow,  col. listfullname, col.x_state ebs_status
...
, ows_req.current_state ows_state,col.casekey, col.alertid, col.rec, col.listrecordtype, col.creation_date, col.listid
...
/* UPDATE OWS INDIVIDUAL (Review EBS Possible) */
...
and col.alertid = ows_req.EXTERNAL_ID
and req.case_state not IN ('D')
and col.x_state like '%Possible' -- Review EBS Possible
and col.x_state <> xwrl_ows_utils.changeowsstate(CURRENT_STATE)
and decode(substr(col.x_state,7),'Positive',4,'Possible',3,'False Positive',2,'Open',1)  >  decode(substr(xwrl_ows_utils.changeowsstate(CURRENT_STATE),7),'Positive',4,'Possible',3,'False Positive',2,'Open',1)
order by ebs_status
;
...
/* UPDATE OWS ENTITY  (Review EBS Possible) */
...
/* UPDATE OWS ENTITY  (Review EBS Positive) */
...
and col.alertid = ows_req.EXTERNAL_ID
and req.case_state not IN ('D')
and col.x_state like '% - Positive' -- Review EBS Possible
and col.x_state <> xwrl_ows_utils.changeowsstate(CURRENT_STATE)
and decode(substr(col.x_state,7),'Positive',4,'Possible',3,'False Positive',2,'Open',1)  >  decode(substr(xwrl_ows_utils.changeowsstate(CURRENT_STATE),7),'Positive',4,'Possible',3,'False Positive',2,'Open',1)
order by ebs_status
;
