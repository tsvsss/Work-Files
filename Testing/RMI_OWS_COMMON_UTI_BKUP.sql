...
AS
   ...
   * 30-APR-2020 IRI              1.44               SAGARWAL       30-APR-2020  E  Added delete_batch Function                               *
   * 30-APR-2020 IRI              1.44               SAGARWAL       30-APR-2020  E  Removed select * from XWRL_REQUESTS                       *
*******************************************************************************************************************************************************/
...
WHERE  ID = :5 ';
   ...
   BEGIN
      ...
      LOOP
         ...
         --         IF get_ows_alert_sequence (r_alert_rec.listrecordtype || ' - False Positive')  >
         --               get_ows_alert_sequence
         --                  (xwrl_ows_utils.changeowsstate
         --                                     (get_ows_alert_state (r_alert_rec.alertid)
         --                                     )
         --                  )
         --         THEN
   ...
   BEGIN
      ...
      EXCEPTION
         ...
         THEN
            ...
            apps.mt_log_error ('Exception occurred while fetching Path: '|| SQLERRM);
      ...
      EXCEPTION
         ...
         THEN
            ...
            apps.mt_log_error('Exception occurred while fetching Batch Details: '|| SQLERRM);
         ...
         LOOP
            ...
            EXCEPTION
               WHEN OTHERS THEN
                  ...
                  apps.mt_log_error('Exception occurred while fetching List ID: '|| SQLERRM);
            ...
            IF l_source_id IS NOT NULL AND l_listid IS NOT NULL AND l_batch_id IS NOT NULL
            THEN
               ...
               LOOP
                  ...
                  apps.mt_log_error ('Processing Alert ID: '|| r_ind_cols.alertid);
                  ...
                  THEN
                     ...
                     apps.mt_log_error ('Changing Alert Id: ' || r_ind_cols.alertid ||' '||'To State: '||r_ind_cols.listrecordtype||l_to_state);

                     l_alert_in_tbl (1).alert_id := r_ind_cols.alertid;
                     l_alert_in_tbl (1).to_state := xwrl_ows_utils.changetoowsstate(r_ind_cols.listrecordtype||l_to_state); --xwrl_ows_utils.changetoowsstate(p_alert_in_tbl (i).p_to_state);
                     ...
                     apps.mt_log_error ('API Status: ' || x_status || ' ' || x_alert_out_tbl(1).err_msg);
                  ...
                  THEN
                     ...
                     INSERT INTO xwrl_alert_notes
                        SELECT xwrl_alert_notes_seq.NEXTVAL,
                               ...
                               r_ind_cols.alertid alert_id, line_number,
         ...
         LOOP
            ...
            THEN
               ...
               LOOP
                  ...
                  apps.mt_log_error ('Processing Alert ID: ' || r_ent_cols.alertid);
                  ...
                  THEN
                     ...
                     apps.mt_log_error ('Changing Alert Id: ' || r_ent_cols.alertid ||' '||'To State: '||r_ent_cols.listrecordtype||l_to_state);

                     l_alert_in_tbl (1).alert_id := r_ent_cols.alertid;
                     l_alert_in_tbl (1).to_state := xwrl_ows_utils.changetoowsstate(r_ent_cols.listrecordtype||l_to_state); --xwrl_ows_utils.changetoowsstate(p_alert_in_tbl (i).p_to_state);
                     l_alert_in_tbl (1).COMMENT  := 'Alert Status To Sync Matches';
                  ...
                  THEN
                     ...
                     INSERT INTO xwrl_alert_notes
                        SELECT xwrl_alert_notes_seq.NEXTVAL,
                               ...
                               r_ent_cols.alertid alert_id, line_number,
                               note, last_update_date, last_updated_by,
                               creation_date, created_by, last_update_login
                          FROM xwrl_alert_notes n
                         WHERE 1 = 1
...
END get_department;
   ...
   BEGIN
      ...
      --SAURABH 16-JAN-2020 Checl OWS Alert State
      --      IF get_ows_alert_sequence (p_alert_state) >
      --            get_ows_alert_sequence
      --               (xwrl_ows_utils.changeowsstate (get_ows_alert_state (p_alert_id)
      --                                              )
      --               )
      --      THEN
      xows.xxiri_cm_process_pkg.update_alerts@ebstoows2.coresys.com
