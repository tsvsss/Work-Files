...
AS
   ...
   * 05-AUG-2020 IRI              1.71               SAGARWAL       05-AUG-2020 B  Fixes in SEND_NOTICE_FROM_TRIGGER                  *
   * 10-AUG-2020 IRI              1.72               SAGARWAL       10-AUG-2020 B  Fixes in DELTE_REQUEST for ESR                     *
   * 10-AUG-2020 IRI              1.73               TSUAZO       10-AUG-2020 B Update Alert Mismatch debugging                    *   
*******************************************************************************************************************************************************/
   ...
   BEGIN
      ...
      LOOP
         ...
         LOOP
            ...
            IF y.SUPPORT_EDOC_ID IS NOT NULL
            THEN
               BEGIN
                  OPEN get_edoc_data (y.SUPPORT_EDOC_ID);
                  ...
                  INSERT INTO xwrl.xwrl_case_documents
                       ...
                       VALUES (xwrl_case_documents_seq.NEXTVAL, x.ID,
                               x.case_id, y.SUPPORT_EDOC_ID, l_edoc_rec.disk_path,
                               '', '',                         -- Doc Category
                               SUBSTR (   'Supporting Doc - '
                                       || y.grade_name
                                       || ' Records',
                                       1,
                                       50
...
--      l_req_details := get_case_details (p_id);
   ...
   IS
      ...
      IS
         ...
         SELECT SUBSTR (xcn.note, 1, 1000) note
          ...
          --SAURABH 05-AUG-2020 T20200803.0051
          WHERE master_id = (SELECT RMI_OWS_COMMON_UTIL.get_master_id(xr.master_id,xr.alias_id,xr.xref_id) FROM XWRL_REQUESTS xr
          WHERE xr.id = p_id);
   ...
   BEGIN
      ...
      --SAURABH 23-JUN-2020 T20200623.0019
      -- For Vessel Contacts send mail to User Registration office Email Address

      IF p_source_table = 'VSSL_CONTACTS_V' THEN
         destination_email_address := rmi_vssl_common_pkg.get_user_register_email(p_created_by);
      ELSIF world_check_iface.is_user_internal (p_created_by)
      THEN
         destination_email_address := get_user_email_address (p_created_by);
      END IF;
...
WHERE  ID = :4 ';
   ...
   BEGIN
      ...
      IF     sanction_status IN ('CONDITIONAL')
         -- ESR Change SAURABH
         AND NVL(ows_rec.department,'XX') = 'SICD'
...
IS
   ...
   l_case_workflow       xwrl_requests.case_workflow%TYPE;
BEGIN
   sub_msg := 'start';
   p_ret_msg := 'Normal';
   p_return_code := 'SUCCESS';
   ...
   THEN
      ...
      THEN
         ...
         BEGIN
            SELECT case_id, case_workflow
              INTO l_case_id , l_case_workflow
              FROM xwrl_requests
             WHERE ID = x_id;
         EXCEPTION
            WHEN OTHERS
         ...
         ELSE
           ...
           -- T20200723.0015 - Trade Compliance Error
           --SAURABH 23-JUL-2020
           IF l_case_workflow NOT IN ('L','SL') THEN
...
END create_ows_generic;
         ...
         SELECT   b.*, xref.start_date, xref.end_date, xref.master_id,
             ...
             FROM xwrl_party_xref xref,
                  (SELECT ID, relationship_type, entity_type, state, status,
                          ...
                          wc_screening_request_id, request_id
   ...
   BEGIN
      ...
      THEN
         ...
         LOOP
            ...
            t_data (ii).request_id := screening_rec.request_id;
            ...
            LOOP
               ...
               t_data (ii).xref_id := NULL;
               t_data (ii).wc_screening_request_id := rec.wc_screening_request_id;
               t_data (ii).request_id := rec.request_id;
               ii := ii + 1;
            END LOOP;
            ...
            LOOP
               ...
               LOOP
                  ...
                  t_data (ii).xref_id := NULL;
                  t_data (ii).wc_screening_request_id := rec1.wc_screening_request_id;
                  t_data (ii).request_id := rec1.request_id;
                  ii := ii + 1;
               END LOOP;
            END LOOP;
         END LOOP;
   ...
   BEGIN
      ...
      THEN
		 ...
		 --SAURABH 22-JUN-2020 T20200525.0023
         -- Fetch all related entities into table type variable
         rmi_ows_common_util.query_cross_reference (p_source_table,
                                                    p_source_table_column,
                                                    p_source_id,
      ...
      THEN
         --
         -- Close ALerts
         -- T20200702.0044 - SAURABH 09-JUL-2020
      FOR rec IN (SELECT ID, case_id, fu.user_name, fu.user_id
                   FROM  xwrl_requests xr, fnd_user fu
                   WHERE batch_id = l_batch_id
                   AND   case_status <> 'C'
                   AND   xr.last_updated_by = fu.user_id
                 )
      LOOP
         close_ows_case (p_user_name       => rec.user_name,--l_username,
      ...
      THEN
         ...
         LOOP
            ...
            THEN
               ...
               apps.mt_log_error (   'Syncing Notes and documents for Req ID '
                                        || l_id
                                       );
                 sync_notes(l_id);
                 sync_documents(l_id);
            ...
            THEN
               ...
               --
               IF entity_data (i).batch_id <> NVL (l_prev_batch_id, -99) AND 1=2
               THEN
...
IS
   ...
   l_current_tc_status         VARCHAR2 (50);
   l_current_expiration_date   DATE;

   -- T20200702.0044 SAURABH 29-JUL-2020
   l_batch_id_existing         NUMBER;
   --- End Code Change

   --- Added by Gopi Vella For T20200804.0009
   l_open_alerts    NUMBER;
   l_path           VARCHAR2(25);
   l_positive_count NUMBER;
   --- End Code Change
BEGIN
   -- T20200722.0029 - FW: OFFICE in TC Screen
   --SAURABH 22-JUL-2020

   IF l_user_id IS NULL THEN

   x_return_status := 'E';
   x_return_msg := 'Invalid user session. Please re-login and try again.';
   ...
   --
   IF entity_data.COUNT > 0                            --AND l_new_batch = 'Y'
   THEN
      -- Loop through each entity and create OWS vetting
      FOR i IN 1 .. entity_data.COUNT
      LOOP
         ...
         x_retrun_msg := NULL;
         l_req := NULL;
         l_current_tc_status := NULL;
         l_current_expiration_date := NULL;
         ...
         IF NVL (entity_data (i).tc_excluded, 'N') != 'Y'
         THEN
            DBMS_OUTPUT.put_line ('Name ' || entity_data (i).full_name);
            l_id := NULL;
            x_return_code := 'SUCCESS';
            x_retrun_msg := NULL;
            l_req := NULL;
            ...
            IF     entity_data (i).alias_id IS NULL
                       ...
                       SELECT ID
                        ...
                        WHERE 1 = 1
                          ...
                          AND xref_id IN (
                                 SELECT ID
                                   FROM xwrl_party_xref
                                  WHERE relationship_master_id =
                                                     entity_data (i).master_id));
            ELSIF     entity_data (i).alias_id IS NOT NULL
                  AND entity_data (i).xref_id IS NULL
            THEN
               SELECT MAX (ID)
                 INTO l_prev_req_id
                 FROM (SELECT ID
                        ...
                        WHERE 1 = 1
                          AND case_state NOT IN ('D', 'E')
                          AND alias_id = entity_data (i).alias_id);
            ELSIF     entity_data (i).alias_id IS NULL
                  AND entity_data (i).xref_id IS NOT NULL
            THEN
               SELECT MAX (ID)
                 INTO l_prev_req_id
                 FROM (SELECT ID
            ...
            IF l_prev_req_id IS NOT NULL
            THEN
               BEGIN
                  SELECT rmi_ows_common_util.case_wf_status (case_workflow),
                         expiration_date
                    INTO l_current_tc_status,
                         l_current_expiration_date
                    FROM xwrl_requests
                   WHERE ID = l_prev_req_id;
               EXCEPTION
                  WHEN OTHERS
                  THEN
                     l_current_tc_status := NULL;
            ...
            DBMS_OUTPUT.put_line ('Prev Batch ' || l_prev_req_id);
            DBMS_OUTPUT.put_line ('TC Status ' || l_current_tc_status);
            DBMS_OUTPUT.put_line ('Date ' || l_current_expiration_date);

            IF     NVL (l_current_tc_status, 'Xx') = 'Provisional' --- Modified by Gopi Vella Removed the Approved condition for HDT T20200804.0009
                   --- IN ('Approved', 'Provisional') 
               AND NVL (l_current_expiration_date, SYSDATE) > SYSDATE
            THEN
               -- Valid Approval available no need to run TC again
               -- T20200722.0010 SAURABH 22-JUL-2020
               x_return_status := 'S';

               -- T20200702.0044 SAURABH 29-JUL-2020
               BEGIN
               SELECT batch_id INTO l_batch_id_existing FROM XWRL_REQUESTS WHERE ID = l_prev_req_id;
               EXCEPTION
               WHEN OTHERS THEN
               NULL;
               END;

               --- Modified by Gopi Vella for HDT T20200804.0009
               --- Checking for the Open Alerts before setting the case workflow to Approved
               --- If there are open alrets can the case workflow cannot be set to approved
               ...
               THEN
                  ...
                  -- T20200624.0036 - Attach user name to Approved TC
                  -- T20200723.0039 - Change Status to Approved from Provisional on Run TC
                  ...
                  UPDATE xwrl_requests
                     SET case_workflow     = 'A',
                         ...
                         last_update_login = l_login_id
                   WHERE 1                 = 1 
                     AND id                = l_prev_req_id;
               
               ELSIF l_open_alerts > 0
               THEN
                  ...
                  THEN
                     ...
                     BEGIN
                        ...
                        SELECT COUNT (1)
                          INTO l_positive_count
                          FROM xwrl_response_ind_columns
                         WHERE request_id = l_prev_req_id
                           AND UPPER (x_state) IN ('EDD - POSITIVE',
                                                   'SAN - OPEN',
                                                   'PEP - POSSIBLE',
                                                   'EDD - OPEN',
                                                   'SAN - POSSIBLE',
            ...
            ELSE
               ...
               THEN
                  ...
                  UPDATE xwrl_requests
                        SET case_state = 'D',
                            ...
                            last_updated_by = l_user_id,
                            last_update_login = l_login_id
                      WHERE 1 = 1 AND ID = l_prev_req_id;

                      DBMS_OUTPUT.put_line ('Request Updated for delete ' || SQL%ROWCOUNT);
               ...
               l_req.batch_id := l_batch_id;
               DBMS_OUTPUT.put_line ('Batch ' || l_batch_id);
               ...
               DBMS_OUTPUT.put_line ('x_id ' || x_id);
               DBMS_OUTPUT.put_line ('x_return_code ' || x_return_code);

               IF x_return_code = 'SUCCESS'
               THEN

                  -- T20200729.0039 SAURABH
                  -- For all departments, Set the status to Processor Review if status is Legal Review

                    --IF l_req.source_table = 'SICD_SEAFARERS' THEN

                   BEGIN
                    UPDATE xwrl_requests
                    SET    case_workflow = rmi_ows_common_util.case_wf_status_dsp('Processor Review')
                    WHERE  ID = l_id
                    AND    case_workflow = rmi_ows_common_util.case_wf_status_dsp('Legal Review')
                    AND    world_check_iface.is_trade_legal_user(l_user_id) = 'N';
                   EXCEPTION
                   WHEN OTHERS THEN
                   apps.mt_log_error (   'Error while updating status '
                                     || l_id||' '|| SQLERRM
                                    );
                   END;
                   ...
                   BEGIN
                    UPDATE xwrl_requests
                    SET    case_workflow = rmi_ows_common_util.case_wf_status_dsp('Provisional')
                    WHERE  ID = l_id
                    AND    case_workflow = rmi_ows_common_util.case_wf_status_dsp('Pending')
                    AND    world_check_iface.is_trade_legal_user(l_user_id) = 'N';
                   EXCEPTION
                   WHEN OTHERS THEN
                   apps.mt_log_error (   'Error while updating status '
                  ...
                  -- Update Batch ID
                  IF entity_data (i).alias_id IS NOT NULL
                  THEN
                     UPDATE xwrl_party_alias
                        SET batch_id = l_batch_id,
                            request_id = l_id,
   ...
   -- T20200702.0044 SAURABH 29-JUL-2020
   x_batch_id := NVL(l_batch_id,l_batch_id_existing);
...
BEGIN
   ...
   LOOP
      ...
      THEN
         ...
         -- TSUAZO 07/20/2020 fix debug statemetn
         ...
         -- TSUAZO 08/10/2020   Modify Debugging
         --APPS.mt_log_error('RMI_OWS_COMMON_UTIL Batch ID: '||l_batch_id||' Request ID: '||p_request_id||' Alert ID: '||x_alert_out_tbl (i).alert_id||' Key Label: '|| x_alert_out_tbl (i).key_label||' Old State: '|| x_alert_out_tbl (i).old_state||' New State: '|| x_alert_out_tbl (i).new_state||' Status: '||x_alert_out_tbl (i).status||' Error: '||x_alert_out_tbl (i).err_msg);                      
              APPS.mt_log_error('RMI_OWS_COMMON_UTIL.CLOSE_OWS_ALERT  Request ID: '||p_request_id||' Alert ID: '||x_alert_out_tbl (i).alert_id||' Key Label: '|| x_alert_out_tbl (i).key_label||' Old State: '|| x_alert_out_tbl (i).old_state||' New State: '|| x_alert_out_tbl (i).new_state||' Status: '||x_alert_out_tbl (i).status||' Error: '||x_alert_out_tbl (i).err_msg);                      
              COMMIT;
...
IS
   ...
   l_userid          NUMBER;
   l_batch_id        NUMBER;
   l_master_id number; -- -- tsuazo 20200714
   x_status          VARCHAR2 (200);
   l_error           VARCHAR2 (500);
   l_path            VARCHAR2 (10);
...
BEGIN
   ...
   BEGIN
      SELECT req.batch_id, req.source_id, req.source_table, req.master_id
        INTO l_batch_id, l_source_id, l_source_table,    l_master_id  -- -- tsuazo 20200714
        FROM xwrl_requests req
         ...
         THEN
            ...
            FOR r_ind_cols IN (SELECT   cols.*
                                  ...
                                  WHERE req.ID = cols.request_id
                                    ...
                                    AND cols.x_state != p_alert_in_tbl (i).p_to_state -- tsuazo 20200714
                     ...
                     /*
                        INSERT INTO xwrl_alert_results_debug
                             ...
                             VALUES ('RMI_OWS_COMMON_UTIL.SYNC_MATCHES:IND',
                                    ...
                                    */
               -- TSUAZO 08/10/2020   Modify Debugging                                    
                APPS.mt_log_error('RMI_OWS_COMMON_UTIL.SYNC_MATCHES:IND Batch ID: '||l_batch_id||' Request ID: '||r_ind_cols.request_id||' Alert ID: '||x_alert_out_tbl (i).alert_id||' Key Label: '|| x_alert_out_tbl (i).key_label||' Old State: '|| x_alert_out_tbl (i).old_state||' New State: '|| x_alert_out_tbl (i).new_state||' Status: '||x_alert_out_tbl (i).status||' Error: '||x_alert_out_tbl (i).err_msg);                      
               COMMIT;
                  ...
                  apps.mt_log_error (  'Update xwrl_response_ind_columns ');
                     ...
                     --WHERE ID = r_ind_cols.ID AND listid = l_listid;   -- tsuazo 20200515 Alert can have multiple response  records
                      ...
                      WHERE  listid = l_listid  and exists  (select 1 from xwrl_requests x where x.id = request_id and x.master_id = l_master_id and x.batch_id = l_batch_id); -- tsuazo 20200714 Alert can have multiple response  records

                     apps.mt_log_error (  'Update xwrl_response_ind_columns - ID: '|| r_ind_cols.ID|| ' UID: ' ||l_listid || ' Alert ID: '||  r_ind_cols.alertid || ' Alert State: '||  p_alert_in_tbl (i).p_to_state ); -- tsuazo 20200528
            ...
            LOOP
             ...
             --xwrl_ows_utils.changetoowsstate(p_alert_in_tbl (i).p_to_state);
                  ...
                  THEN
                     ...
                     -- TSUAZO 08/10/2020   Modify Debugging                                             
                     APPS.mt_log_error('RMI_OWS_COMMON_UTIL.SYNC_MATCHES:ENTITY Batch ID: '||l_batch_id||' Request ID: '||r_ent_cols.request_id||' Alert ID: '||x_alert_out_tbl (i).alert_id||' Key Label: '|| x_alert_out_tbl (i).key_label||' Old State: '|| x_alert_out_tbl (i).old_state||' New State: '|| x_alert_out_tbl (i).new_state||' Status: '||x_alert_out_tbl (i).status||' Error: '||x_alert_out_tbl (i).err_msg);                      
                     COMMIT;                                    
                  ...
                  apps.mt_log_error (  'ENTITY Update xwrl_response_entity_columns ');
                     ...
                     --WHERE ID = r_ind_cols.ID AND listid = l_listid;   -- tsuazo 20200515 Alert can have multiple response  records
                      ...
                      apps.mt_log_error (  'ENTITY Update xwrl_response_entity_columns - ID: '|| r_ent_cols.ID|| ' UID: ' ||l_listid || ' Alert ID: '|| r_ent_cols.alertid || ' Alert State: '||  p_alert_in_tbl (i).p_to_state ); -- tsuazo 20200528
...
BEGIN
   ...
   FOR rec IN (SELECT ID, master_id, alias_id, xref_id
                 FROM xwrl_requests
                WHERE ID = p_request_id)
   LOOP
      ...
      FOR r_alias_rec IN (SELECT xr.ID, xr.case_id, xr.batch_id,
                            ...
                            FROM xwrl_requests xr
                           WHERE batch_id = p_batch_id
                           AND RMI_OWS_COMMON_UTIL.get_master_id(master_id, alias_id, xref_id) = RMI_OWS_COMMON_UTIL.get_master_id(rec.master_id,rec.alias_id,rec.xref_id)
                           AND id <> rec.id
                                               )
      LOOP
         apps.mt_log_error ('Inside Alias Rec...');
...
PROCEDURE sync_notes (p_request_id IN INTEGER)
IS
   l_doc_rec     xwrl_case_documents%ROWTYPE;
   l_id          NUMBER;
   l_notes_rec   xwrl_case_notes%ROWTYPE;
BEGIN
   apps.mt_log_error ('Inside Sync Notes...');
   COMMIT;

   FOR rec IN (SELECT ID, master_id, alias_id, xref_id, case_id
                 FROM xwrl_requests
                WHERE ID = p_request_id)
   LOOP

      FOR recnote IN (SELECT MAX (ID) request_id
                        FROM xwrl_requests x
                       WHERE ID < p_request_id
                         AND get_master_id (x.master_id, x.alias_id,
                                            x.xref_id) =
                                get_master_id (rec.master_id,
                                               rec.alias_id,
                                               rec.xref_id
                                              ))
      LOOP

         apps.mt_log_error ('Past Request ID for '||rec.id||' is '||recnote.request_id);
         COMMIT;

         FOR r_primary_rec IN (SELECT *
                                 FROM xwrl_case_notes
                                WHERE request_id = recnote.request_id)
         LOOP
            apps.mt_log_error ('Inside Primary Rec...Notes');
            COMMIT;
            l_notes_rec := r_primary_rec;
            ...
            l_notes_rec.ID := l_id;
            l_notes_rec.case_id := rec.case_id;
            l_notes_rec.request_id := rec.ID;
            apps.mt_log_error (   'Request ID: '
                               || rec.ID
                               || ' '
                               || 'Case ID: '
...
PROCEDURE sync_documents (p_request_id IN INTEGER)
IS
   l_doc_rec     xwrl_case_documents%ROWTYPE;
   l_id          NUMBER;
BEGIN
   apps.mt_log_error ('Inside Sync Docs...');
   COMMIT;
   ...
   LOOP
      ...
      LOOP
         ...
         FOR r_primary_rec IN (SELECT *
                                ...
                                WHERE request_id = recnote.request_id)
         LOOP
            apps.mt_log_error ('Inside Primary Rec...Docs');
            COMMIT;
            l_doc_rec := r_primary_rec;
            ...
            l_doc_rec.ID := l_id;
            l_doc_rec.case_id := rec.case_id;
            l_doc_rec.request_id := rec.ID;
            apps.mt_log_error (   'Request ID: '
                               || rec.ID
                               || ' '
                               || 'Case ID: '
...
BEGIN
   ...
   THEN
      ...
      END;
	   ...
	   ELSIF l_source_table = 'ESR_REQUEST_USER_DETAILS'
   THEN
      dept := 'Corporate';
      dept_ext := 'ESR';

      BEGIN
         SELECT ESR_REQUEST_USER_DETAILS_ID
           INTO v_esr_user
           FROM ESR_REQUEST_USER_DETAILS
          WHERE ESR_REQUEST_USER_DETAILS_ID = l_source_id;

         IF v_esr_user IS NOT NULL
         THEN
            dept_ext := 'ESR( User Id: ' || v_esr_user || ')';
         END IF;
      EXCEPTION
         WHEN NO_DATA_FOUND
         THEN
...
IS
   ...
   IS
      ...
      SELECT CASE
                ...
                ELSE fnd_profile.value_specific
                                             (NAME         => 'ONT_DEFAULT_PERSON_ID',
                                              user_id      => req.created_by
                                             )
             END salesrep_id
...
END update_alert_status_gender;*/
   ...
   ---Code Modified By Gopi Vella For HDT T20190515.0016
   ...
   BEGIN
      ...
      ELSE
         ...
         IF l_ows_data.city_of_residence_id IS NOT NULL --l_ows_data.city IS NOT NULL
         THEN

            OPEN cur_city_check (l_ows_data.city_of_residence_id); --(rmi_ows_common_util.get_city_list_id (l_ows_data.city));
   ...
   EXCEPTION
      WHEN OTHERS THEN
         ...
         mt_log_error ('Exception occurred while checking for Crimean entity in rmi_ows_common_util.is_entity_crimean ' || SQLERRM);

   END is_entity_crimean;
...
IS
   ...
   IS
      SELECT --* SAURABH Remove *
      name_screened, id, xref_id, alias_id, master_id,batch_id,source_table
        FROM xwrl_requests
...
BEGIN
   ...
   IF p_target_data.source_table = 'CORP_MAIN' THEN
   
   UPDATE RMI_CORP_ESR_INFO
   SET    OWS_CORP_ESR_REQ_ID = p_source_data.ID,
          OWS_CORP_REQ_BATCH_ID = p_source_data.batch_id
   WHERE  OWS_CORP_ESR_REQ_ID = p_target_data.id;
   
   
   
   UPDATE RMI_CORP_ESR_INFO
   SET    VOID_OWS_CORP_ESR_REQ_ID = p_source_data.ID,
          VOID_OWS_CORP_REQ_BATCH_ID = p_source_data.batch_id
   WHERE  VOID_OWS_CORP_ESR_REQ_ID = p_target_data.id;
   
   ELSIF p_target_data.source_table = 'ESR_REQUEST_USER_DETAILS' THEN
   UPDATE RMI_CORP_ESR_INFO
   SET    OWS_ESR_REQ_ID = p_source_data.ID
   WHERE  OWS_ESR_REQ_ID = p_target_data.id;
   
   
   
   UPDATE RMI_CORP_ESR_INFO
   SET    VOID_OWS_ESR_REQ_ID = p_source_data.ID
   WHERE  VOID_OWS_ESR_REQ_ID = p_target_data.id;
   END IF;
...
PROCEDURE getDPCDetails (
    p_source_table_id IN      NUMBER,
    p_master_id       IN      NUMBER,
    p_xref_id         IN      NUMBER,
    p_alias_id        IN      NUMBER,
    p_source_table    IN      VARCHAR2,
    l_consent_id    OUT     NUMBER,
    l_name          OUT     VARCHAR2,
    l_dob           OUT     DATE,
    l_entity        OUT     VARCHAR2,
    l_nationality   OUT     VARCHAR2
) AS

BEGIN

   IF p_source_table_id IS NOT NULL
   THEN
      ...
      THEN
         BEGIN
            SELECT consent_id
              INTO l_consent_id
              FROM rmi_data_privacy_consent
             WHERE entity_id = p_source_table_id;
         EXCEPTION
            WHEN NO_DATA_FOUND
            THEN
               l_consent_id := NULL;
      ...
      ELSE
         -- SAURABH OWS
         --
         IF p_xref_id IS NULL AND p_alias_id IS NULL
         THEN
            BEGIN
               SELECT MAX (consent_id)
                 INTO l_consent_id
                 FROM rmi_data_privacy_consent
                WHERE 1 = 1              --entity_id = :GLOBAL.source_table_id
         ...
         ELSE
            BEGIN
               SELECT MAX (consent_id)
                 ...
                 FROM rmi_data_privacy_consent
                WHERE 1 = 1              --entity_id = :GLOBAL.source_table_id
                  AND master_id = (SELECT relationship_master_id
                                     FROM xwrl_party_xref
                                    WHERE ID = p_xref_id);
            EXCEPTION
               WHEN NO_DATA_FOUND
      ...
      IF l_consent_id IS NULL
      THEN

         IF p_source_table = 'SICD_SEAFARERS'
         THEN

            -- SAURABH 20-MAY-2020 T20200519.0003
            IF p_master_id IS NOT NULL
            THEN
            ...
            ELSE
               BEGIN
                  SELECT TRUNC (birth_date)
                    INTO l_dob
                    FROM sicd_seafarers
                   WHERE seafarer_id = p_source_table_id;
               EXCEPTION
                  WHEN OTHERS
                  THEN
                     l_dob := NULL;
               ...
               BEGIN
                  SELECT l_ctry.country_name
                    INTO l_nationality
                    FROM sicd_seafarers a, sicd_countries l_ctry
                   WHERE seafarer_id = p_source_table_id
                     AND nationality = l_ctry.country_code;
               EXCEPTION
                  WHEN OTHERS
                  THEN
                     l_nationality := NULL;
            ...
            IF     l_dob IS NOT NULL
               AND l_nationality IS NOT NULL
               AND l_name IS NOT NULL
            THEN
               l_consent_id :=
                          rmi_consent_id (l_name, l_nationality, l_dob, NULL);
            ...
            END IF;
         --
         -- SAURABH 30-JAN-2020 T20191219.0012
         --
         ELSIF p_source_table = 'CORP_MAIN'
         THEN
            IF     p_master_id IS NOT NULL
            ...
            THEN
               BEGIN
                  SELECT TRIM (full_name)
                    INTO l_name
                    FROM xwrl_party_master a, xwrl_party_xref b
                   WHERE 1 = 1
                     AND a.ID = b.relationship_master_id
            ...
            BEGIN
               SELECT MAX (consent_id)
                 ...
                 FROM rmi_data_privacy_consent
                WHERE entity_id = p_source_table_id
                  AND first_name = l_name
                  AND entity_type = 'CORPORATION';
            EXCEPTION
               WHEN OTHERS
               THEN
                  l_consent_id := NULL;
...
PROCEDURE sync_master_to_tc (p_request_id IN NUMBER)
IS
   CURSOR cur_tc
   IS
      SELECT master_id, alias_id, xref_id,
             COUNTRY_OF_ADDRESS,
...
BEGIN
   FOR rec IN cur_tc
   LOOP
      l_master_id := get_master_id (rec.master_id, rec.alias_id, rec.xref_id);

      UPDATE xwrl_party_master
         SET passport_issuing_country_code = rec.COUNTRY_OF_ADDRESS,
             country_of_residence = rec.country_of_residence,
             citizenship_country_code = rec.COUNTRY_OF_NATIONALITY,
             city_of_residence_id = rec.city_of_residence_id,
             date_of_birth =
                        REPLACE (SUBSTR (rec.date_of_birth, 1, 10), '-', NULL),
             sex = DECODE (rec.gender, 'M', 'MALE', 'F', 'FEMALE', 'UNKNOWN'),
       ...
       UPDATE xwrl_requests x
       SET   COUNTRY_OF_ADDRESS = rec.COUNTRY_OF_ADDRESS,
             country_of_residence = rec.country_of_residence,
             COUNTRY_OF_NATIONALITY =  rec.COUNTRY_OF_NATIONALITY,
             city_of_residence_id = rec.city_of_residence_id,
             date_of_birth = rec.date_of_birth,
             gender = rec.gender,
             passport_number =  rec.passport_number
       WHERE get_master_id (x.master_id, x.alias_id, x.xref_id) = get_master_id (rec.master_id, rec.alias_id, rec.xref_id)
       AND   id <> p_request_id
       AND   case_status = 'O';
   ...
   COMMIT;
END sync_master_to_tc;
--

--

PROCEDURE create_esr_corp_batch_vetting (
   p_source_table          IN       VARCHAR2,
   p_source_table_column   IN       VARCHAR2,
   p_source_id             IN       NUMBER,
...
BEGIN
   ...
   DBMS_OUTPUT.put_line ('Start');
   -- Fetch all related entities into table type variable
   rmi_ows_common_util.query_esr_corp_cross_reference (p_source_table,
                                              p_source_table_column,
                                              p_source_id,
                                              p_corp_name,
                                              entity_data
   ...
   IF entity_data.COUNT = 0
   THEN
      insert_esr_corp_party_master (p_source_table,
                           p_source_table_column,
                           p_source_id,
                           p_corp_name,
                           x_id
   ...
   THEN
      ...
      LOOP
         ...
         THEN
            ...
            IF     NVL (l_current_tc_status, 'Xx') IN
            ...
            THEN
               ...
               BEGIN
               SELECT batch_id INTO l_batch_id FROM XWRL_REQUESTS WHERE ID = l_prev_req_id;
               EXCEPTION
               WHEN OTHERS THEN
               NULL;
               END;
...
EXCEPTION
   ...
   THEN
      ...
      x_return_status := 'E';
      x_return_msg := 'Exception while submitting OWS Batch ' || SQLERRM;
END create_esr_corp_batch_vetting;


   PROCEDURE query_esr_corp_cross_reference (
      p_source_table    IN       VARCHAR2,
      p_source_column   IN       VARCHAR2,
      p_source_id       IN       NUMBER,
      p_corp_name          IN    VARCHAR2,
         ...
         (SELECT   *
            ...
            WHERE 1 = 1
              ...
              AND source_id =
               ...
               and upper(full_name)  = upper(p_corp_name)
         ORDER BY r.creation_date DESC)
         WHERE ROWNUM =1 ;
      ...
      THEN
         ...
         LOOP
            ...
            t_data (ii).wc_screening_request_id :=
             ...
             t_data (ii).department := screening_rec.department;
                  t_data (ii).priority := screening_rec.priority;
                  t_data (ii).document_type := screening_rec.document_type;

         END LOOP;

      END IF;
   END query_esr_corp_cross_reference;

   PROCEDURE insert_esr_corp_party_master (
      p_source_table          IN       VARCHAR2,
      p_source_table_column   IN       VARCHAR2,
      p_source_id             IN       NUMBER,
   ...
   BEGIN
      ...
      SELECT COUNT (1)
        ...
        FROM xwrl_party_master
       WHERE status = 'Enabled'
       and upper(full_name)  = upper(p_corpname)
         AND relationship_type = 'Primary'
         AND source_table = p_source_table
         AND source_table_column = p_source_table_column
         AND source_id = p_source_id;

   fnd_file.put_line(fnd_file.LOG,'insert party master::l_cnt ::'|| l_cnt);
      ...
      THEN
         ...
         THEN
            ...
            LOOP
               ...
               l_party_rec.sex := NULL;
               l_party_rec.imo_number := NULL;
               l_party_rec.department := 'CORP';
               l_party_rec.office := NULL;
               l_party_rec.priority := '3';
               l_party_rec.risk_level := NULL;
               l_party_rec.document_type := 'ESR';
               l_party_rec.closed_date := NULL;
               l_party_rec.assigned_to := NULL;
               l_party_rec.vessel_indicator := NULL;
               l_party_rec.passport_number := NULL;
         ...
         l_party_rec := NULL;
         --
         fnd_file.put_line(fnd_file.LOG,'insert party master lcnt = 0::x_id ::'|| x_id);

        /* insert_esr_corp_cross_ref (p_source_table,
                                  p_source_table_column,
                                  p_source_id,
                                  x_id
                                 ); */
       ...
       WHERE status = 'Enabled'
        and upper(full_name)  = upper(p_corpname)
         ...
         fnd_file.put_line(fnd_file.LOG,'insert party master else lcnt = 0::l_cnt ::'|| l_cnt);

         if l_cnt >0 AND p_source_id IS NOT NULL
         and p_source_table = 'CORP_MAIN'
         then
           SELECT MIN (ID)
      INTO x_id
      FROM xwrl_party_master
      ...
      AND source_id         = p_source_id;
         ...
         fnd_file.put_line(fnd_file.LOG,'insert party master else lcnt > 0::x_id ::'|| x_id);
         end if;
	...
	IF p_source_table = 'CORP_MAIN'
         THEN
            ...
            COMMIT;
              x_id := l_party_rec.ID;
               fnd_file.put_line(fnd_file.LOG,'insert party master else lcnt > 0 new insert::x_id ::'|| x_id);
            END IF;
		...
		END IF;
         DBMS_OUTPUT.put_line ('Master exists');
      END IF;
   END insert_esr_corp_party_master;

   PROCEDURE insert_esr_corp_cross_ref (
      p_source_table          IN   VARCHAR2,
      p_source_table_column   IN   VARCHAR2,
      p_source_id             IN   NUMBER,
      p_id                    IN   NUMBER
   ...
   EXCEPTION
      ...
      THEN
         NULL;
   END insert_esr_corp_cross_ref;

PROCEDURE insert_esr_user_master (p_esr_id IN NUMBER, p_esr_action IN VARCHAR2,p_username IN VARCHAR2)
IS
   CURSOR cur_esr
   IS
      SELECT *
   ...
   l_master_id   NUMBER;
   x_id          NUMBER;
   l_usr_sub_esr_addr_ctry VARCHAR2 (40);
   l_usr_det_id  NUMBER;
   l_fullname    VARCHAR2(500);
   l_gender      varchar2(20);
   l_address_country varchar2(20);
   l_nationality varchar2(100);
   l_city_id      number;
   l_corp_name   varchar2(300);
   l_nationality_code varchar2(10);
BEGIN
   --
   FOR rec IN cur_esr
   LOOP
	  ...
	  if p_esr_action = 'Create'
	  then
	    l_usr_det_id := rec.ESR_REQUEST_USER_DETAILS_ID;
		l_fullname   := rec.USER_SUBMITTING_ESR;

                l_address_country := upper(rec.ESR_USR_COUNTRY_CODE);
                l_nationality:= upper(rec.USER_SUBMTG_ESR_NATIONALITY);
                l_city_id := rec.ESR_USR_CITY_ID;

                select decode(rec.USER_SUBMTG_ESR_GENDER,'Male', 'M',
                                                                'Female', 'F',
                                                                'Unspecified', NULL,
                                                                 NULL,NULL) into l_gender from dual;

	  end if;

	  if p_esr_action = 'Void'
	  then
	    l_usr_det_id := rec.VOID_ESR_USER_DETAILS_ID;
		l_fullname   := rec.VOID_ESR_USR;

                l_address_country := upper(rec.VOID_ESR_USR_COUNTRY_CODE);
                l_nationality:= upper(rec.VOID_ESR_USR_NATIONALITY);
                l_city_id := rec.VOID_ESR_USR_CITY_ID;

                 select decode(rec.VOID_ESR_USR_GENDER,'Male', 'M',
                                                                'Female', 'F',
                                                                'Unspecified', NULL,
                                                                 NULL,NULL) into l_gender from dual;
	  end if;
      ...
      SELECT COUNT (1)
        ...
        FROM xwrl_party_master
       WHERE status = 'Enabled'
         and TRIM (upper(full_name)) = upper(p_username)
         AND relationship_type = 'Primary'
         AND source_table = 'ESR_REQUEST_USER_DETAILS'
         AND source_table_column = 'ESR_REQUEST_USER_DETAILS_ID'
         AND source_id = l_usr_det_id;

      IF l_cnt = 0 AND p_esr_id IS NOT NULL
      THEN

	  BEGIN

        select iso_alpha2_code into l_nationality_code
                from SICD_COUNTRIES
                WHERE STATUS = 'Active'
                and iso_alpha2_code is not null
                and country_name = l_nationality;
        ...
        END;
         ...
         l_party_rec.state := 'Verified';
         l_party_rec.status := 'Enabled';
         l_party_rec.source_table := 'ESR_REQUEST_USER_DETAILS';
         l_party_rec.source_table_column := 'ESR_REQUEST_USER_DETAILS_ID';
         l_party_rec.source_id := l_usr_det_id;
         l_party_rec.xref_source_table := NULL;
         l_party_rec.xref_source_table_column := NULL;
         l_party_rec.xref_source_id := NULL;
         l_party_rec.full_name := l_fullname;
         l_party_rec.family_name := NULL;
         l_party_rec.given_name := NULL;
         l_party_rec.date_of_birth := NULL;
         l_party_rec.sex := l_gender;
         l_party_rec.imo_number := NULL;
         l_party_rec.department := 'CORP';
         l_party_rec.office := NULL;
         l_party_rec.priority := '3';
         ...
         l_party_rec.passport_number := NULL;
         l_party_rec.passport_issuing_country_code := NULL;
         l_party_rec.citizenship_country_code := l_nationality_code;
         l_party_rec.country_of_residence := l_address_country;
         l_party_rec.city_of_residence_id := l_city_id;
         l_party_rec.note := NULL;
         l_party_rec.start_date := SYSDATE;
         l_party_rec.end_date := NULL;
         l_party_rec.last_update_date := SYSDATE;
      ...
      BEGIN
        ...
        select corp_name into l_corp_name
        from rmi_corp_esr_info
        where esr_id = p_esr_id;
         ...
         SELECT MIN (ID)
          ...
          WHERE source_table = 'CORP_MAIN'
            AND source_id = rec.corp_id
            and upper(full_name)  = upper(l_corp_name)
            AND relationship_type = 'Primary'
            AND entity_type = 'ORGANISATION';
      EXCEPTION
         WHEN OTHERS
      ...
      dbms_output.put_line('Master ID'||l_master_id);

      IF l_master_id IS NOT NULL
      THEN
         insert_esr_user_cross_ref ('CORP_MAIN',
                                  'CORP_ID',
                                  rec.corp_id,
                                  l_master_id,
				p_esr_action,
      ...
      ELSE
         insert_esr_corp_party_master ('CORP_MAIN', 'CORP_ID', rec.corp_id,l_corp_name, x_id);

      dbms_output.put_line('X ID'||x_id);
   ...
   END LOOP;
END insert_esr_user_master;

-------------------------------------------------------------------------------------

 PROCEDURE insert_esr_user_cross_ref (
   p_source_table          IN   VARCHAR2,
   p_source_table_column   IN   VARCHAR2,
   p_source_id             IN   NUMBER,
   p_id                    IN   NUMBER,
   p_esr_action            IN VARCHAR2,
   p_esr_id             IN  NUMBER
)
IS
   --
...
FOR recesr IN (SELECT *
         ...
         LOOP
		  ...
		  if p_esr_action = 'Create'
	  then
	    l_usr_det_id := recesr.ESR_REQUEST_USER_DETAILS_ID;
		l_fullname   := recesr.USER_SUBMITTING_ESR;

                l_address_country := upper(recesr.ESR_USR_COUNTRY_CODE);
                l_nationality:= upper(recesr.USER_SUBMTG_ESR_NATIONALITY);
                l_city_id := recesr.ESR_USR_CITY_ID;

                select decode(recesr.USER_SUBMTG_ESR_GENDER,'Male', 'M',
                                                                'Female', 'F',
                                                                'Unspecified', NULL,
                                                                 NULL,NULL) into l_gender from dual;
	  ...
	  if p_esr_action = 'Void'
	  then
	    l_usr_det_id := recesr.VOID_ESR_USER_DETAILS_ID;
		l_fullname   := recesr.VOID_ESR_USR;

                l_address_country := upper(recesr.VOID_ESR_USR_COUNTRY_CODE);
                l_nationality:= upper(recesr.VOID_ESR_USR_NATIONALITY);
                l_city_id := recesr.VOID_ESR_USR_CITY_ID;

                 select decode(recesr.VOID_ESR_USR_GENDER,'Male', 'M',
                                                                'Female', 'F',
                                                                'Unspecified', NULL,
                                                                 NULL,NULL) into l_gender from dual;
	  end if;
	       ...
	       BEGIN
            ...
            dbms_output.put_line('Esr'||recesr.esr_id);
            BEGIN
               ...
               SELECT MAX (ID)
                 ...
                 FROM xwrl_party_master xpm
                WHERE status = 'Enabled'
        and TRIM (upper(full_name)) = upper(l_fullname)
         AND relationship_type = 'Primary'
                  AND xpm.source_id = l_usr_det_id
                  AND xpm.source_table = 'ESR_REQUEST_USER_DETAILS'
                  AND xpm.source_table_column = 'ESR_REQUEST_USER_DETAILS_ID';
            EXCEPTION
               WHEN OTHERS
               THEN
                  lv_id := NULL;
