...
* 06-AUG-2020 IRI              1.18                TSUAZO        06-AUG-2020 IRI    I     Add timing columns    * 
********************************************************************************************************************/
...
commit;   -- tsuazo 11/4/2019 confirming this commit is at the end of the loop.  Note. This was moved to outside the loop.
  ...
  -- Initialize id
	...
	-- Initialize request record
     ...
     v_request_rec.ebs_start := sysdate; -- TSUAZO 08/06/2020
  ...
  -- Save record attributes
   ...
   -- Send Request 
	...
	--- Get Response 
      ...
      END IF;
      v_request_rec.ows_start := sysdate; -- TSUAZO 08/06/2020
      v_resp := utl_http.get_response (v_req);
      v_request_rec.ows_end := sysdate; -- TSUAZO 08/06/2020
      IF p_debug THEN
   ...
   -- Parse Response to save data
    ...
    INTO
     ...
     WHERE id = v_id;
      ...
      ,ebs_end = sysdate -- TSUAZO 08/06/2020
      WHERE id = v_id;
      COMMIT; -- TSUAZO 11/17/2019
