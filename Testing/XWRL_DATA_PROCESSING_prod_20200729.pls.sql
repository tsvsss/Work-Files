...
********************************************************************************************************************/
      ...
      FUNCTION get_case_state (p_status IN xwrl_parameters.KEY%TYPE)
      RETURN VARCHAR2
   IS
   ...
   BEGIN
      SELECT value_string
       ...
       WHERE ID = 'CASE_STATE' AND KEY = p_status;
   ...
   END get_case_state;
         ...
         FUNCTION get_case_status (p_status IN xwrl_parameters.KEY%TYPE)
      RETURN VARCHAR2
   IS
      l_return_value   xwrl_parameters.KEY%TYPE;
   BEGIN
      SELECT value_string
        INTO l_return_value
        FROM xwrl_parameters
       WHERE ID = 'CASE_STATUS' AND KEY = p_status;
   ...
   FUNCTION get_office(p_code IN vssl_offices.code%TYPE) RETURN VARCHAR2 IS
   l_return_value vssl_offices.description%TYPE;
   ...
   EXCEPTION
      ...
      THEN
         ...
         FUNCTION get_case_workflow (p_status IN xwrl_parameters.KEY%TYPE)
      RETURN VARCHAR2
   IS
      l_return_value   xwrl_parameters.KEY%TYPE;
   BEGIN
   ...
   EXCEPTION
      ...
      THEN
         RETURN NULL;
   END get_case_workflow; 
   
   
   FUNCTION contains_legal_review (p_path IN varchar2, p_request_id IN number) return varchar2 is
   ...
   FUNCTION is_maintenance_window (p_date date) RETURN boolean as
