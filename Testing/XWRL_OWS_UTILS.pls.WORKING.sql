...
-- PROCESS INDIVIDUAL ALERT RESPONSE
            ...
            IF     v_path = 'INDIVIDUAL'   AND l_alert_out_tbl (i).new_state IS NOT NULL THEN
              ...
              apps.mt_log_error('xwrl_ows_utils  -- INDIVIDUALS - r_cols loop:'||SQLERRM);
              end;
...
ELSIF     v_path = 'ENTITY'    AND l_alert_out_tbl (i).new_state IS NOT NULL THEN
              ...
              when others then 
              --error_message := SQLERRM;
              apps.mt_log_error('xwrl_ows_utils  -- ENTITY - r_cols loop:'||SQLERRM);
              end;

               END IF;  --- END IF V_PATH

         END LOOP;  -- END L_ALERT_OUT

      END IF;  -- END IF L_ALERT_IN_TBL
