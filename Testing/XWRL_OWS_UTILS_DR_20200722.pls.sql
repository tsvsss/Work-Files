      ...
      * 20-JUL-2020 IRI                 1.13               TSUAZO              20-JUL-2020 B  Update commit statements          *   
       * 22-JUL-2020 IRI                 1.14               TSUAZO              22-JUL-2020 B  Fix issue with auto_clear array          * 
             * 22-JUL-2020 IRI                 1.15               TSUAZO              22-JUL-2020 B  Fix issue with auto_clear query          *   
                          * 23-JUL-2020 IRI                 1.16               TSUAZO              23-JUL-2020 B  Fix issue with auto_clear query - remove col.id from select distinct  ADD COUNT FROM XWRL_ALERT_NOTES - should have included REC as a col         *   
********************************************************************************************************************/
...
-- tsuazo 20200717
            ...
            IF (l_alert_out_tbl (i).status = 'SUCCESS')  THEN 
               ...
               IF     v_path = 'INDIVIDUAL'
                  AND l_alert_out_tbl (i).new_state IS NOT NULL
               THEN
               ...
               and alert_id = l_alert_out_tbl (i).alert_id
               ...
               THEN
                ...
                BEGIN               
               -- TSUAZO 20200717 Add loop for RESPONSE ENTITY
                FOR r_cols IN ( 
                      select unique i.request_id, r.case_id,i.alertid,listid, i.x_state, r.name_screened
...
--         END IF;
      ...
      THEN
         ...
         LOOP
            ...
            INSERT INTO xwrl_alerts_debug
                 ...
                 VALUES ('XWRL_OWS_UTILS.PROCESS_ALERTS(2)', v_user,
                         l_alert_in_tbl (i).alert_id,
                         l_alert_in_tbl (i).to_state,
                         l_alert_in_tbl (i).COMMENT, SYSDATE, p_user_id
                        );
            ...
            */
            ...
            THEN
               INSERT INTO xwrl_alert_results_debug
                    ...
                    VALUES ('XWRL_OWS_UTILS.PROCESS_ALERTS(2)',
                            p_request_id, v_case_key,
                            l_alert_out_tbl (i).alert_id, v_list_id,
                            l_alert_out_tbl (i).key_label,
                            l_alert_out_tbl (i).old_state,
   ...
   IS
      ...
      */
      ...
      IS
         SELECT DISTINCT col.request_id, r.source_table, r.source_id,
...
--            AND col.request_id = p_request_id;
   ...
   BEGIN
      ...
      LOOP
         v_count := v_count + 1;
         /* Note: this was hard set to the first record
         p_alert_tab (1).p_alert_id := c1rec.alertid;
         p_alert_tab (1).p_to_state := c1rec.to_state;
         p_alert_tab (1).p_comment := c1rec.note;*/
         -- tsuazo 20200722 fix issue with the array
      ...
      IS
         SELECT DISTINCT col.request_id, r.source_table, r.source_id,
                col.listid, col.alertid, col.x_state
                                                    ...
                                                    --SAURABH 13-JAN-2020
                ,
                col.listrecordtype || SUBSTR (CLEAR.to_state, 4) to_state
                                                                         -- TSUAZO 20-FEB-2020
                                                                                          --,clear.to_state
                ,
                CLEAR.note, CLEAR.max_id
         ...
         SELECT DISTINCT col.request_id, r.source_table, r.source_id,
           ...
           FROM xwrl_response_entity_columns col,
                ...
                (WITH max_tab AS
                 ...
                 SELECT x.source_table, x.source_id, x.list_id, x.to_state,
                        x.note, x.master_id, x.alias_id, x.xref_id,
                        r.relationship_master_id, max_tab.ID max_id
