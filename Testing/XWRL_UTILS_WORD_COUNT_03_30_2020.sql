...
* 17-MAR-2020 IRI              1.12                TSUAZO        17-MAR-2020  I     ADD word count     * 
********************************************************************************************************************/
   ...
   BEGIN
     ...
     v_process := true;
	  v_word_count := 3;
       ...
       -- tsuazo MatchRule Filter
       begin
        select nvl(mst.number_of_words,3) 
        into v_word_count
		...
		if  v_word_count <= 2 and c1rec.MatchRule = '[I220O] Additional names typo tolerant only' then
		    v_process := false;
                 ...
                 select count(*)
                   ...
                   AND UPPER(value_string) = UPPER(c1rec.Category); 
...
commit;   -- tsuazo 11/4/2019 confirming this commit is at the end of the loop.  Note. This was moved to outside the loop.
   ...
   BEGIN
      ...
      if  v_rec.path = 'INDIVIDUAL'  then

           select count(*)
           into v_rec_count
           from xwrl_response_ind_columns
           where request_id = c1rec.id
           and rec = c1rec.rec_row;
