...
select *
from worldcheck_external_xref
where wc_screening_request_id = 79;

select *
from worldcheck_external_xref
where source_table = 'CORP_MAIN'
and source_table_id = 1055719
order by worldcheck_external_xref_id desc
;

select *
from xwrl_response_entity_columns
where alertid  = 'SEN-9793338'
;
...
where case_id = 9793338
;

xwrl_utils;


select batch_id, name_screened, department_ext, office, case_status, case_state, case_workflow
from xwrl_requests 
where trunc(creation_date) = trunc(sysdate)
and case_status = 'O'
order by id desc;
...
select *
from xwrl_response_ind_columns
where request_id = 2316
--and last_updated_by = 1156
;


update xwrl_response_ind_columns
set legal_review = null
where request_id = 88183
and last_updated_by = 1156
;

select matchrule, 'INDIVIDUAL', count(*)
from xwrl_response_ind_columns
group by matchrule
union
select matchrule, 'ENTITY', count(*)
from xwrl_response_entity_columns
group by matchrule
order by 3 desc
;
...
select xwrl_utils.cleanse_name(full_name)
FROM xwrl.xwrl_party_master
;

select xwrl_utils.cleanse_name(UPPER(ltrim(first_name||' '||last_name)))
from rmi_data_privacy_consent
;

UPDATE rmi_data_privacy_consent a
   SET master_id = (SELECT MAX(ID) FROM xwrl.UPDATE rmi_data_privacy_consent a
   SET master_id = (SELECT MAX(ID) FROM xwrl.xwrl_party_master WHERE xwrl_utils.cleanse_name(full_name) = xwrl_utils.cleanse_name(UPPER(ltrim(a.first_name||' '||a.last_name))))
WHERE master_id IS NULL;

UPDATE rmi_data_privacy_consent a
   SET master_id = (SELECT MAX(MASTER_ID) FROM xwrl.xwrl_party_alias WHERE xwrl_utils.cleanse_name(full_name) = xwrl_utils.cleanse_name(UPPER(ltrim(a.first_name||' '||a.last_name))))
WHERE master_id IS NULL; WHERE xwrl_utils.cleanse_name(full_name) = xwrl_utils.cleanse_name(UPPER(ltrim(a.first_name||' '||a.last_name))))
WHERE master_id IS NULL;
...
select a.*,xwrl_utils.cleanse_name(UPPER(a.first_name||' '||a.middle_name||' '||a.last_name))
...
from rmi_data_privacy_consent a
,xwrl_party_master b
WHERE xwrl_utils.cleanse_name(b.full_name) = xwrl_utils.cleanse_name(UPPER(ltrim(a.first_name)||ltrim(' '||a.middle_name)||LTRIM(' '||a.last_name)))
...
select *
from xwrl_requests
where batch_id = 45720;


select *
from fnd_user
where user_id in (8270,6435)
;
