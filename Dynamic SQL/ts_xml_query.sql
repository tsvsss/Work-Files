...
alter system kill session '1874,29803' immediate;
...
begin
dbms_scheduler.drop_job(job_name => 'TC_OWS_DATA_I_2019030631_JOB');
end;
/

begin
dbms_scheduler.drop_job(job_name => 'TC_OWS_DATA_E_2019030636_JOB');
end;
/
...
SELECT XMLQUERY(  
    replace('declare namespace ws="http://www.datanomic.com/ws"; copy $tmp := .  modify (replace value of node $tmp//*/ws:request/ws:EntityName with "BM&A CONSULTANTS LTD" ) return $tmp' ,'&',chr(38)||'amp;')
...
SELECT 'dbms_scheduler.DEFINE_PROGRAM_ARGUMENT(program_name => v_program_name, argument_name => '||''''||x.value||''''||', argument_position => '||to_char(x.rec+22)||', argument_type => '||''''||'VARCHAR2'||''''||',default_value => '||''''||''''||');'
from xwrl_parameters t,
       XMLTABLE(XMLNAMESPACES( 'http://schemas.xmlsoap.org/soap/envelope' as "soapenv", 'http://www.datanomic.com/ws' as "ws"),
         '//ws:*'
         PASSING t.value_xml
...
SELECT 'dbms_scheduler.DEFINE_PROGRAM_ARGUMENT(program_name => v_program_name, argument_name => '||''''||x.value||''''||', argument_position => '||to_char(x.rec+24)||', argument_type => '||''''||'VARCHAR2'||''''||',default_value => '||''''||''''||');'
from xwrl_parameters t,
       XMLTABLE(XMLNAMESPACES( 'http://schemas.xmlsoap.org/soap/envelope' as "soapenv", 'http://www.datanomic.com/ws' as "ws"),
         '//ws:*'
         PASSING t.value_xml
...
select 'INSERT INTO XWRL.XWRL_PARAMETERS (ID, KEY, VALUE_STRING,SORT_ORDER) VALUES ('||''''||'RESPONSE_ROWS'||''''||','||''''||x.value||''''||');'
--SELECT x.value||' varchar2(2700), '
...
select *
from wc_screening_request
where wc_screening_request_id = 39513;


-- Note: the XREF table links Request to EBS table to identify relationship (e.g. Lawyer to Corporations)
-- This does not always reflect something like Individual to specific seafarer
select *
from WORLDCHECK_EXTERNAL_XREF
where wc_screening_request_id = 819852
order by 1 desc
;
   ...
   SELECT  xref.wc_screening_request_id,count(*)
   FROM
      wc_screening_request wsr,
      worldcheck_external_xref xref,
      xwrl_requests xr
   WHERE wsr.wc_screening_request_id = xref.wc_screening_request_id (+)
   AND  wsr.wc_screening_request_id = xr.wc_screening_request_id (+)
      AND wsr.entity_type = 'INDIVIDUAL'
      AND trunc (wsr.creation_date) >= trunc (SYSDATE) - 30
      AND xr.wc_screening_request_id is null
...
select log_id, JOB_name, status, additional_info, output
from all_scheduler_job_run_details
--where status = 'FAILED'
where job_name like 'OWS%'
--WHERE job_name like 'PROCESS%'
...
select *
from all_scheduler_job_run_details
where job_name = 'OWS_R_2019030726_31_JOB'
ORDER BY 1 DESC
;
...
SELECT SQL||''''||','||ROWNUM*10||');', value
FROM (
select 'INSERT INTO XWRL.XWRL_PARAMETERS (ID, KEY, VALUE_STRING,SORT_ORDER,DISPLAY_FLAG) VALUES ('||''''||'RESPONSE_ROWS'||''''||','||''''||x.value||''''||','||''''||'TEMPNAME'||''''||','||''''||'N' SQL, x.value
--SELECT x.value||' varchar2(2700), '
-- SELECT x.value||' varchar2(2700) path '||''''||'dn:'||x.value||''''||',' 
--select 'x.'||x.value||','
--select 'v_rec.'||x.value||' := c1rec.'||x.value||';'
...
SELECT '||CHR(39)||'||chr(39)||code||chr(39)||'||CHR(39)||'||chr(39)||','||chr(39)
FROM VSSL_STATUSES
where unique_record_identifier = 'Y'
ORDER BY SORT_BY
;
...
select ' and status in ('||CHR(39)||'AC'||CHR(39)||','||CHR(39)||'A'||CHR(39)||','||CHR(39)||'SU'||CHR(39)||','||CHR(39)||'RV'||CHR(39)||','||CHR(39)||'S'||CHR(39)||',)'
from dual;