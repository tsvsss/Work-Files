...
with org as (select x.wc_screening_request_id 
from  WC_SCREENING_REQUEST x
where x.entity_type = 'ORGANISATION')
select c.*
from wc_content c
,org
where c.wc_screening_request_id =  org.wc_screening_request_id;


with org as (select x.wc_screening_request_id 
from  WC_SCREENING_REQUEST x
where x.entity_type = 'ORGANISATION')
select unique c.notes
from wc_content c
,org
where c.wc_screening_request_id =  org.wc_screening_request_id;
...
select *
from wc_content_details_v
--where WC_SCREENING_REQUEST_ID = :WC_SCREENING_REQUEST_ID
;


select xmlquery('declare namespace soap="http://schemas.xmlsoap.org/soap/envelope"; 
declare namespace ns2="http://screening.complinet.com"; 
         //*/names'
                 PASSING c.xml_response  RETURNING content) xml
from wc_content c
...
SELECT 'dbms_scheduler.DEFINE_PROGRAM_ARGUMENT(program_name => v_program_name, argument_name => '||''''||x.value||''''||', argument_position => '||to_char(x.rec+7)||', argument_type => '||''''||'VARCHAR2'||''''||',default_value => '||''''||''''||');'
from xwrl_parameters t,
       XMLTABLE(XMLNAMESPACES( 'http://schemas.xmlsoap.org/soap/envelope' as "soapenv", 'http://www.datanomic.com/ws' as "ws"),
         '//ws:*'
         PASSING t.value_xml
